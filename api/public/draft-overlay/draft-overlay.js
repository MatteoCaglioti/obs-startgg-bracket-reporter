...
const PICK_BASE = { w: 210, h: 263 };
const BAN_BASE  = { w: 120, h: 150 };
...
socket.on('draft:update', (state) => {
  ...
  renderSlots(bansA,  (leftIsA ? state.teamABans  : state.teamBBans ).map(find).filter(Boolean), false, BAN_BASE,  false);
  renderSlots(picksB, (leftIsA ? state.teamBPicks : state.teamAPicks).map(find).filter(Boolean), true,  PICK_BASE, true);
  renderSlots(bansB,  (leftIsA ? state.teamBBans  : state.teamABans ).map(find).filter(Boolean), true,  BAN_BASE,  false);
